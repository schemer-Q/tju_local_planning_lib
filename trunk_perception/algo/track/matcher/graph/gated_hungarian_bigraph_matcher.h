...
#include "trunk_perception/algo/track/matcher/graph/connected_component_analysis.h"
#include "trunk_perception/algo/track/matcher/graph/hungarian_optimizer.h"
...
class GatedHungarianMatcher {
 public:
  ...
  void Match(T cost_thresh, OptimizeFlag opt_flag, std::vector<std::pair<size_t, size_t>>* assignments,
             std::vector<size_t>* unassigned_rows, std::vector<size_t>* unassigned_cols);

  void Match(T cost_thresh, T bound_value, OptimizeFlag opt_flag, std::vector<std::pair<size_t, size_t>>* assignments,
             std::vector<size_t>* unassigned_rows, std::vector<size_t>* unassigned_cols);
...
void GatedHungarianMatcher<T>::Match(T cost_thresh, T bound_value, OptimizeFlag opt_flag,
                                     ...
                                     std::vector<size_t>* unassigned_rows, std::vector<size_t>* unassigned_cols) {
  if (!assignments) {
    TFATAL << "[GatedHungarianMatcher] assignments is nullptr!";
    return;
  }

  if (!unassigned_rows) {
    TFATAL << "[GatedHungarianMatcher] unassigned_rows is nullptr!";
    return;
  }

  if (!unassigned_cols) {
    TFATAL << "[GatedHungarianMatcher] unassigned_cols is nullptr!";
    return;
  }
  ...
  if (row_components.size() != col_components.size()) {
    TFATAL << "[GatedHungarianMatcher] row_components size and col_components size is not equal!";
    return;
  }
...
void GatedHungarianMatcher<T>::MatchInit() {
  ...
  if (find_ret == compare_fun_map.end()) {
    TFATAL << "[GatedHungarianMatcher] opt_flag is error!";
    return;
  }
  ...
  if (is_valid_cost_(bound_value_)) {
    TFATAL << "[GatedHungarianMatcher] bound_value is invalid!";
    return;
  }
}
...
void GatedHungarianMatcher<T>::ComputeConnectedComponents(std::vector<std::vector<size_t>>* row_components,
                                                          std::vector<std::vector<size_t>>* col_components) const {
  if (!row_components) {
    TFATAL << "[GatedHungarianMatcher] row_components is nullptr!";
    return;
  }

  if (!col_components) {
    TFATAL << "[GatedHungarianMatcher] col_components is nullptr!";
    return;
  }
...
void GatedHungarianMatcher<T>::OptimizeAdapter(std::vector<std::pair<size_t, size_t>>* local_assignments) {
  if (!local_assignments) {
    TFATAL << "[GatedHungarianMatcher] local_assignments is nullptr!";
    return;
  }
