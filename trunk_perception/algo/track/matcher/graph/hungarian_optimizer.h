...
#include "trunk_perception/algo/track/matcher/graph/secure_matrix.h"
...
class HungarianOptimizer {
 ...
 private:
  ...
  bool IsStarred(const size_t row, const size_t col) const { return marks_(row, col) == Mark::STAR; }
  ...
  bool IsPrimed(const size_t row, const size_t col) const { return marks_(row, col) == Mark::PRIME; }
