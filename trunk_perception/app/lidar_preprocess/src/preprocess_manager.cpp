...
std::shared_ptr<PreprocessBase> PreprocessManager::Create(const YAML::Node& config) {
  if (!config["Type"].IsDefined() || !config["Config"].IsDefined()) {
    TFATAL << "PreprocessManager::Create: config is invalid";
    return nullptr;
  }

  std::shared_ptr<PreprocessBase> processor = nullptr;

  if (config["Type"].as<std::string>() == "BASE") {
    processor = std::make_shared<PreprocessBase>();
  } else {
...
std::shared_ptr<PreprocessBase> PreprocessManager::Create(const std::string& file) {
