...
float ExistenceFusion1L1R::Compute(const FusedObject::Ptr& fused_object_ptr) {
  ...
  if (score < 0.3 && fused_object_ptr->lidar_total_life > 30 && fused_object_ptr->front_radar_consecutive_hit > 10 &&
	...
	// @author zzg 2024_12_04 增加逻辑：目标类型为 TRUCK、VEHICLE，激光连续命中 15 帧以上，然后激光测量丢失 12s 以内的，且毫米波一直连续命中 25 帧以上，，则维持融合目标
	...
	if ( (score < 0.3) && (fused_object_ptr->front_radar_consecutive_hit > 25) && (fused_object_ptr->lidar_consecutive_hit_his > 15) &&
			((fused_object_ptr->timestamp - fused_object_ptr->lidar_consecutive_hit_his_ts) < 12.0) ) {
		if (fused_object_ptr->type == ObjectType::TRUCK || fused_object_ptr->type == ObjectType::VEHICLE) {
			TINFO << " fused_object_ptr->odo_lidar_ptr:\n" << fused_object_ptr->odo_lidar_ptr->Matrix(); 
			...
			std::cout << " 1l1r id: " << fused_object_ptr->track_id << "  temp_center.x():" << temp_center.x() << " temp_center.y():" << temp_center.y() << std::endl;
			if (temp_center.x() < 25 && std::fabs(temp_center.y()) < 2.0 ) {
				score = 0.3;
				std::cout << " id: " << fused_object_ptr->track_id << "  score:" << score << std::endl;
