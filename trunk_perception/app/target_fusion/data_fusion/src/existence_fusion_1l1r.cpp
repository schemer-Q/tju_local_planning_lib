...
float ExistenceFusion1L1R::Compute(const FusedObject::Ptr& fused_object_ptr) {
  ...
  // @author zzg 2024_12_04 增加逻辑：目标类型为 TRUCK、VEHICLE， 激光雷达总生命周期 35 帧以上，且 毫米波一直连续命中 25 帧以上，则维持融合目标
  // 为了解决近距离怼脸货车由于 激光连续丢失多帧 导致的目标丢失；   解决特定问题
  if ( (score < 0.3) && (fused_object_ptr->front_radar_consecutive_hit > 25) && (fused_object_ptr->lidar_total_life > 35)) {
