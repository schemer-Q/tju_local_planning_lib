...
ExistenceFusion1L5R1V::ExistenceFusion1L5R1V(const ExistenceFusionConfig::ConstPtr& config)
    : ExistenceFusionBase(config) {}

float ExistenceFusion1L5R1V::Compute(const FusedObject::Ptr& fused_object_ptr) {
  if (fused_object_ptr == nullptr) {
    TERROR << "ExistenceFusion1L5R1V::Compute: fused_object_ptr is nullptr";
    return 0.0;
  }
...
bool ExistenceFusion1L5R1V::IsObjectHasNFrameRadar(const FusedObject::Ptr& fused_object_ptr, int num) {
  const int& front_radar_consecutive_hit = fused_object_ptr->front_radar_consecutive_hit;
